...
* **Command Prefix:** By default, commands start with `.` (e.g., `.help`, `.ping`). This can be changed using
  `.setprefix`.
* **Inline Bot:** Use the userbot's features inline by mentioning its associated bot username (obtained during setup or
  via `.ch_hikka_bot`).
* **Help:** Use the `.help` command to see available modules and commands. Use `.help <module_name>` for specific module
  help.
* **Configuration:** Use the `.config` command for an interactive menu to configure modules and core settings.

## License

This project is licensed under the **GNU Affero General Public License v3.0 (AGPLv3)**. See
the [LICENSE](https://www.gnu.org/licenses/agpl-3.0.html) file for details.
